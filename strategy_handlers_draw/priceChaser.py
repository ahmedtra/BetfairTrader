...
from strategy_handlers_draw.utils import get_price_market_selection, place_bet, replace_order, get_placed_orders
from time import sleep

class PriceChaser():
    def __init__(self, client, market_id, selection_id):
        get_logger().debug("creating Price chaser", market_id = market_id, selection_id = selection_id)
        self.client = client
    ...
    def chasePrice(self, price, size, side, ignore_current_position = False):
        get_logger().info("start chasing", price = price, size = size, side = side)
        ...
        if not tradable:
            ...
            return None
        betfair_position = 0
        if not ignore_current_position:
